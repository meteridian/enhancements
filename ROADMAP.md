...
| [METR-0013](enhancements/0013-unit-economics/unit-economics.md) | Unit Economics and Business Telemetry | draft |
...
| [ADR-0020](docs/adr/0020-focus-cost-data-interchange.md) | FOCUS v1.4 as canonical cost data interchange format | Accepted |
| [ADR-0021](docs/adr/0021-unlimited-custom-dimensions.md) | Unlimited custom dimensions via CloudEvents extension attributes | Accepted |
...
- OCP, AWS, Azure, GCP provider support (Tier 1 native integrations)
- FOCUS v1.4 ingestion and export — accept any provider's FOCUS-format billing data without custom integration (ADR-0020)
- Generic FOCUS CSV upload and API ingestion from v1.0 (Tier Custom)
...
- Unit economics: Business telemetry ingestion, unit cost calculation, multi-dimensional views, block-extensible cost allocation (METR-0013)
- Unlimited custom dimensions on metering events via CloudEvents extension attributes (ADR-0021)
...
## Phase 1.1: Tier 2 Provider Expansion (v1.1)

Target: Extend native integrations to major non-hyperscaler clouds.

| Provider | Integration type | Notes |
|----------|-----------------|-------|
| IBM Cloud | Native source block | Cost and Usage Reports API |
| Oracle Cloud | Native source block | OCI Cost Analysis API |
| Alibaba Cloud | Native source block | Billing API |
| Huawei Cloud | Native source block | BSS API |

All Tier 2 providers also export FOCUS-format data, so the generic FOCUS
ingestion path (available from v1.0) provides immediate coverage while native
integrations are developed for richer metadata and real-time metering.

---

## Phase 1.2: Tier 3 SaaS Integrations (v1.2+)

Target: Extend metering to SaaS and AI platform providers.

| Provider | Category | Integration type |
|----------|----------|-----------------|
| OpenAI | AI and LLM | API usage polling |
| Anthropic | AI and LLM | API usage polling |
| Datadog | Observability | Billing API |
| Snowflake | Data platform | Usage history views |
| Databricks | Data platform | Billable usage API |
| MongoDB Atlas | Database | Billing API |
| Confluent | Streaming | Billing metrics API |
| Vercel | Platform | Usage API |
| Cloudflare | CDN and edge | GraphQL analytics |
| GitHub | Developer tools | Actions and Copilot usage API |

Additional SaaS integrations are community-driven via the block marketplace
(METR-0002). Any provider that exports FOCUS-format data can be ingested
immediately via the Tier Custom path without waiting for a native integration.
