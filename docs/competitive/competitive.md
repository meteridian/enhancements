...
| Telco BSS (charging/billing) | $20-30B | Amdocs, Netcracker/NEC, Ericsson, Nokia | Not primary target; architecturally capable of slice-based metering |
...
| **Cost distribution** | Basic | Basic | Basic | Basic | Basic | **Advanced (production-proven)** |
...
2. **Cost distribution**: Production-proven platform/worker/storage/network/GPU
   distribution logic is unmatched. No FinOps tool does proportional overhead
   distribution — and users can plug in custom Transform blocks for arbitrary
   allocation algorithms (METR-0002).
...
| **K8s cost allocation** | Excellent | Good | Good | Good | **Excellent (production-proven)** |
| **Multi-cluster** | Pro only | Community | Yes | Yes | **Yes** |
| **Automated optimization** | Recommendations | None | Autonomous | ML-based | **Via robne/Kubernaut** |
| **GPU cost tracking** | Yes | Basic | Yes | No | **Yes (MIG-aware)** |
| **Non-K8s costs** | AWS/Azure/GCP | No | No | No | **All infrastructure** |
| **Billing/chargeback** | Basic export | No | No | No | **Full billing pipeline** |
...
| **5G/slicing** | Yes | Yes | Yes | Yes | Yes | Yes | Yes | **Architecturally capable (slice as metering dimension)** |
...
**Meteridian is not targeting telco BSS.** It does not implement 3GPP protocols,
CDR mediation, Diameter interfaces, or GSMA roaming settlement. However, its
architecture provides telco-grade billing accuracy (exactly-once processing,
real-time balance management, complex rating) for the **underserved middle**:
organizations that need that accuracy without telco-grade infrastructure costs.

Network slices can be modeled as a metering dimension (custom CloudEvents
attribute) rather than as a native protocol-level concept. This makes
Meteridian architecturally capable of slice-based billing without being a
telco BSS platform.

Target customers in this space:
...
- Neoclouds offering BMaaS, VMaaS, DBaaS, GPUaaS, and MaaS

Key advantage: **open source + infrastructure-native + pluggable block
architecture**. No telco BSS handles bare-metal metering, Kubernetes cost
allocation, or GPU billing. No telco BSS is open source. And no telco BSS
allows users to swap pipeline components via a block marketplace.
...
3. **Production-proven distribution logic** for GPU cost allocation (distribute
   GPU node costs to consuming namespaces proportionally via pluggable blocks)
...
### 3. Advanced Cost Distribution (Pluggable Blocks)
Platform overhead, worker unallocated, storage, network, and GPU costs
distributed proportionally to consuming projects — using pluggable Transform
blocks (METR-0002) that users can replace with custom allocation algorithms
(ML-based attribution, multi-factor regression, etc.).
