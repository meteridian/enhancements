...
architecture** ([METR-0002](../0002-extensibility/extensibility.md)). Users can
swap, extend, or replace almost any component by plugging in their own blocks.
The platform provides financial-grade plumbing (data plane, serialization,
credit ledger, product catalog, rating engine, trust framework) and lets users
assemble the pipeline that fits their business.
...
| **Managed Service Providers (MSPs)** | Billing customers for hosted services (VMs, K8s, GPUs) |
| **Neoclouds and Sovereign Clouds** | BMaaS, VMaaS, DBaaS, GPUaaS, MaaS, and similar XaaS |
| **B2B and B2C subscription management** | Native subscription support, or integration with Zuora, Lago, and Stripe Billing via Sink blocks |

Meteridian is **not** targeting telco BSS (no 3GPP, no CDR mediation, no
Diameter) — though its architecture is capable of slice-based metering as a
dimension.
...
5. Provide a pluggable block architecture where every pipeline stage is replaceable
...
3. Requiring blockchain infrastructure for core functionality
4. Replacing existing monitoring systems (consume their data instead)
5. Native telco BSS (3GPP, CDR mediation, Diameter)
...
7. **Backward compatibility:** Migration path from Red Hat Cost Management for existing customers
