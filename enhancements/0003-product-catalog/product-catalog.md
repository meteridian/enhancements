...
### 4.6 FOCUS Column Mapping

The Resource Type Registry is designed to be bidirectionally mappable to and from
the [FOCUS v1.4](https://focus.finops.org/) (FinOps Open Cost and Usage
Specification) schema. This enables Meteridian to ingest FOCUS-format billing
data from any provider and to export usage and cost data in FOCUS format for
interoperability with external FinOps tools.

Key FOCUS columns and their Meteridian equivalents:

| FOCUS column | Meteridian equivalent | Notes |
|--------------|----------------------|-------|
| `ServiceName` | `catalog.products.name` | Top-level product grouping |
| `ServiceCategory` | `catalog.products.category` | compute, storage, network, gpu, platform |
| `ResourceType` | `catalog.resource_types.name` | Canonical resource vocabulary |
| `PricingUnit` | `catalog.resource_types.unit_of_measure` | core-hours, GB-months, calls |
| `PricingQuantity` | Metric output `quantity` | Computed by SQL-based metric definitions |
| `BilledCost` | Rated event `amount` | Output of the rating block |
| `EffectiveCost` | Rated event `effective_amount` | After discounts and credits |
| `ListUnitPrice` | `catalog.tiers.unit_price` or `catalog.charges.unit_price` | Published rate |
| `ChargeCategory` | Derived from `catalog.charges.charge_model` | Usage, Purchase, Tax, Credit, Adjustment |
| `CommitmentDiscountStatus` | Credit ledger state | Used, Unused, or N/A |

The `resource_type_mappings` table (section 4.3) supports a `provider = 'focus'`
entry for each resource type, defining how to extract quantities from
FOCUS-format CSV columns. This means any provider that exports in FOCUS format
can be ingested without a custom integration — the FOCUS provider mapping handles
the translation generically.

See [ADR-0020](../../docs/adr/0020-focus-cost-data-interchange.md) for the full
FOCUS adoption decision and ingestion pipeline design.
