...
> **Catalog drives pluggable block pipelines:** Catalog entries determine which
> blocks execute in the pipeline. A product's charge model selects the rating
> block; a plan's configuration can route events through custom Transform blocks
> for enrichment, cost allocation, or business logic. Users can replace the
> built-in rating block with a custom implementation for exotic pricing models
> (see §12.4) — the catalog is the control plane for the block dataflow.
