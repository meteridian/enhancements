...
- **Related:** METR-0002 (Extensibility), METR-0004 (Credit, Prepaid, and Token Billing), METR-0005 (Internal Budget Units), METR-0006 (Developer Experience), METR-0013 (Unit Economics), ADR-0021 (Unlimited Custom Dimensions)
...
### 4.6 Custom Dimensions for Resource Types

Resource type mappings and metrics can reference **unlimited custom dimensions**
attached to metering events via CloudEvents extension attributes (see
[ADR-0021](../../docs/adr/0021-unlimited-custom-dimensions.md)). Custom
dimensions use the `mtrn` prefix on CloudEvents extension attributes and are
stored in a JSONB column (`custom_dimensions`) with GIN indexing, enabling
zero-configuration addition of new attribution axes at any time.

This means that resource types and their associated metrics are not limited to
fixed tags or pre-registered labels. Any business-relevant dimension —
`customer_id`, `feature_name`, `deployment_region`, `model_version`,
`cost_center`, `team`, `environment`, or any user-defined attribute — can be
attached to events and immediately used in metric queries, rate plan evaluation,
and cost allocation without schema migrations, code deployments, or enablement
configuration.

**Why this matters for the Product Catalog:**

- **Fine-grained cost allocation.** Charges and metrics can group and filter by
  arbitrary dimension combinations. A single `cpu_core_hours` resource type can
  produce per-customer, per-feature, per-region cost breakdowns — all from the
  same event stream.

- **No dimension limits.** Traditional cost management platforms cap custom
  dimensions at 20 to 40 enabled tags per provider type. Meteridian imposes no
  such limit. Organizations can attach as many dimensions as their events
  require, and every dimension is queryable from day one.

- **Provider mapping enrichment.** Resource type mappings (Section 4.3) can
  include custom dimensions in their `quantity_expr` and `filter_expr` fields,
  enabling provider-specific cost attribution that goes beyond what the raw
  cloud billing data provides.

**Example: filtering a resource type mapping by custom dimension**

```sql
INSERT INTO catalog.resource_type_mappings
    (resource_type_id, provider, event_type, quantity_expr, filter_expr)
VALUES (
    (SELECT id FROM catalog.resource_types WHERE name = 'gpu_hours'),
    'ocp',
    'gpu_usage',
    'gpu_hours',
    'custom_dimensions @> ''{"model_version": "v3"}'''
);
```

This mapping counts GPU hours only for events tagged with `model_version=v3`,
enabling per-model-version pricing without any change to the resource type
registry schema.

See [METR-0013 (Unit Economics)](../0013-unit-economics/unit-economics.md) for
how custom dimensions feed into unit cost calculations, and
[ADR-0021](../../docs/adr/0021-unlimited-custom-dimensions.md) for the full
design rationale and storage schema.
...
#### GPU Cost by Customer and Model Version (Custom Dimensions)

SQL-based metrics can reference any custom dimension from the event's JSONB
`custom_dimensions` column (ADR-0021). No prior registration is required —
the dimension is available as soon as events carry it.

```sql
-- metric: gpu_cost_by_customer_model (v1)
-- parameters: start_time (timestamptz), end_time (timestamptz)
-- output: customer (text), model_version (text), quantity (numeric)

SELECT
    custom_dimensions->>'customer' AS customer,
    custom_dimensions->>'model_version' AS model_version,
    SUM(gpu_hours) AS quantity
FROM events.gpu_usage
WHERE usage_start >= :start_time
  AND usage_start < :end_time
  AND custom_dimensions ? 'customer'
GROUP BY
    custom_dimensions->>'customer',
    custom_dimensions->>'model_version';
```

This metric produces per-customer, per-model-version GPU consumption without
any schema change or configuration update. The `custom_dimensions` GIN index
ensures efficient filtering even as the number of distinct dimension keys grows.
