...
10. [Revenue Recognition](#10-revenue-recognition)
...
- **At-grant valuation**: Credits are valued at the price paid when the grant was
  created. Used for revenue recognition per ASC 606 and IFRS 15.
...
Credit-based billing creates specific revenue recognition obligations under both
ASC 606 (US GAAP, mandatory in the United States) and IFRS 15 (International
Financial Reporting Standards, mandatory in the EU, UK, Australia, Canada, India,
Japan, Brazil, and approximately 140 other countries). Both standards define
substantially the same 5-step model for revenue recognition, but a global billing
platform must comply with both.

### 10.1 The 5-Step Model (ASC 606 and IFRS 15)

Both standards require:

1. **Identify the contract** — the committed spend contract or credit purchase agreement
2. **Identify performance obligations** — each billable service or resource type
3. **Determine the transaction price** — the credit grant amount in fiat currency
4. **Allocate the transaction price** — distribute across performance obligations
5. **Recognize revenue when obligations are satisfied** — as credits are consumed

### 10.2 Deferred Revenue (Contract Liability)

When a customer purchases credits (prepaid), the payment is recorded as **deferred
revenue** (a liability on the balance sheet), not as recognized revenue. Revenue is
recognized only when credits are consumed against actual usage.

**Ledger mechanics:**

| Event | Debit | Credit | Revenue Impact |
|-------|-------|--------|----------------|
| Credit purchase ($10,000) | Cash | Deferred Revenue | None (liability created) |
| Credit consumption ($500) | Deferred Revenue | Revenue | $500 recognized |
| Credit expiry ($200) | Deferred Revenue | Revenue (breakage) | $200 recognized |

The credit ledger (Section 3.3) provides the source data for deferred revenue
tracking. Each `grant` transaction creates a deferred revenue entry, and each
`consumption` or `expiry` transaction triggers partial recognition.

**Breakage estimation:** Under both ASC 606 and IFRS 15, if credits are expected
to expire unused (breakage), the entity may recognize that revenue proportionally
over the consumption period rather than at expiry. Meteridian tracks historical
breakage rates per tenant and per grant source to support this estimation.

### 10.3 Unbilled Revenue (Contract Asset)

When usage occurs before invoicing (common in arrears billing or when credit
drawdown reports are issued monthly), the usage is recognized as **unbilled
revenue** (a contract asset). This represents revenue earned but not yet invoiced.

**Ledger mechanics:**

| Event | Debit | Credit | Revenue Impact |
|-------|-------|--------|----------------|
| Usage delivered, not yet invoiced | Unbilled Revenue (Asset) | Revenue | Revenue recognized |
| Invoice generated | Accounts Receivable | Unbilled Revenue | Asset reclassified |

The credit ledger tracks consumption in real time. The unbilled revenue accrual is
computed at period end as: total consumption since last invoice minus already-invoiced
amounts.

### 10.4 Multi-Element Arrangements

Committed spend contracts (Section 4) often bundle multiple performance obligations
with different delivery timelines — for example, a contract granting compute credits,
storage credits, and premium support. Under ASC 606 and IFRS 15, the transaction
price must be allocated to each distinct performance obligation based on standalone
selling prices.

Meteridian supports this through:

- **Product catalog linkage** (METR-0003): Each credit type maps to a product
  catalog entry with a standalone selling price
- **Allocation at grant time**: When a bundled contract is created, the total
  contract value is allocated to each credit type proportionally
- **Independent recognition**: Each credit type's deferred revenue is recognized
  independently as that specific credit type is consumed

### 10.5 Revenue Schedule Generation

Meteridian generates periodic revenue schedules that summarize recognized,
deferred, and unbilled revenue for each tenant and contract:

```
GET /api/v1/credits/revenue-schedule?tenant_id={uuid}&period=2026-07
```
...
```json
{
  "tenant_id": "uuid",
  "period": "2026-07",
  "recognized_revenue": 4500.00,
  "deferred_revenue_opening": 10000.00,
  "deferred_revenue_closing": 5300.00,
  "unbilled_revenue": 200.00,
  "breakage_recognized": 0.00,
  "currency": "USD",
  "by_performance_obligation": [
    {
      "product": "compute_credits",
      "recognized": 3200.00,
      "deferred_closing": 2800.00
    },
    {
      "product": "storage_credits",
      "recognized": 1300.00,
      "deferred_closing": 2500.00
    }
  ]
}
```

### 10.6 Journal Entry Export

For integration with ERP systems (SAP, Oracle Financials, NetSuite, Xero,
QuickBooks), Meteridian exports revenue recognition journal entries in standard
formats:
...
|--------|--------------|-------------|
| CSV (configurable) | Generic and QuickBooks | Column-mapped journal entries |
| JSON (structured) | REST API consumers | Programmatic integration |
| SAP BAPI-compatible | SAP ERP and S/4HANA | FI posting format |
| Oracle GL interface | Oracle Financials | GL_INTERFACE table format |
| NetSuite CSV | NetSuite | Journal import format |

**Export API:**

```
POST /api/v1/credits/journal-entries/export
```
...
Each journal entry includes the accounting date, debit and credit accounts,
amount, currency, reference to the source transaction (grant ID, consumption
event ID), and a narrative description suitable for auditor review.

### 10.7 Compliance with ASC 606 and IFRS 15

**Key differences between the standards that Meteridian handles:**

| Area | ASC 606 (US GAAP) | IFRS 15 (International) | Meteridian Approach |
|------|-------------------|------------------------|---------------------|
| Variable consideration constraint | Constraint applies to avoid significant revenue reversal | Highly probable threshold for inclusion | Configurable threshold per tenant jurisdiction |
| Breakage | Recognize proportionally if expected, at expiry otherwise | Same principle, less prescriptive guidance | Tenant-configurable breakage method (proportional or point-in-time) |
| Contract modifications | Prospective or cumulative catch-up depending on type | Similar but with additional scope-change guidance | Tracks modification history on contracts for both methods |
| Disclosure requirements | ASC 606-10-50 specific quantitative disclosures | IFRS 15.110-129 with some differences in granularity | Generates disclosure data for both standards |

The tenant's jurisdiction setting determines which standard governs the revenue
schedule output format and disclosure data generation. Multi-entity tenants
operating under both GAAP and IFRS can generate dual reports.
