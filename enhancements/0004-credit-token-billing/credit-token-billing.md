...
10. [Real-Time Pre-Authorization Wallet](#10-real-time-pre-authorization-wallet)
...
## 10. Real-Time Pre-Authorization Wallet

### 10.1 Motivation

Current wallet implementations (Meteridian included) check balances at billing time (post-hoc). Enterprise use cases require pre-authorization: verifying sufficient balance BEFORE serving a request, with atomic reservation to prevent overdraft across concurrent requests.

No competitor (Orb, Metronome, Lago) offers sub-10ms pre-authorization with atomic guarantees. Orb explicitly acknowledges this gap.

### 10.2 Architecture

Pre-auth uses Valkey (Redis-compatible) for sub-millisecond atomic operations:
...
type PreAuthResponse struct {
    ...
    RemainingBal  decimal.Decimal `json:"remaining_balance"`
    ExpiresAt     time.Time `json:"expires_at"`
}
```

Flow:
1. Client calls `POST /api/v1/wallets/{id}/pre-authorize` with amount
2. Valkey WATCH + MULTI/EXEC atomically: check balance >= amount, decrement available, create reservation
3. If approved, return reservation_id (TTL-bounded)
4. On usage completion: `POST /api/v1/wallets/reservations/{id}/capture` (convert to actual debit)
5. On timeout/cancel: `POST /api/v1/wallets/reservations/{id}/release` (return to available)

### 10.3 Concurrency Guarantees

- Atomic check-and-decrement via Valkey transactions (no race conditions)
- Reservation TTL prevents indefinite holds (configurable, default 5 minutes)
- Expired reservations auto-release via Valkey TTL keys
- Consistency: Valkey is source of truth for available balance; PostgreSQL is source of truth for ledger (async reconciliation)

### 10.4 Pooled and Shared Wallets

Multi-entity wallet sharing for enterprise accounts:
- Parent wallet with sub-allocations per business unit
- Hierarchical limits: BU cannot exceed its allocation, total cannot exceed parent
- Cross-entity transfer without double-entry (internal reallocation)

### 10.5 API

```
POST /api/v1/wallets/{id}/pre-authorize         # Reserve amount
POST /api/v1/wallets/reservations/{id}/capture   # Confirm usage
POST /api/v1/wallets/reservations/{id}/release   # Cancel reservation
GET  /api/v1/wallets/{id}/reservations           # Active reservations
POST /api/v1/wallets/{id}/pools                  # Create shared pool
```

### 10.6 Integration with Entitlements (METR-0011)

Pre-auth result feeds into the ext_authz decision:
- If pre-auth approved → allow request
- If pre-auth denied (insufficient balance) → reject with 402 or trigger enforcement policy
- Enables true pay-per-request models with zero overdraft risk
