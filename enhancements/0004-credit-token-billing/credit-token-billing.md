...
> **Pluggable block architecture (METR-0002):** The credit and billing system is
> composed of standard Meteridian blocks. Payment collection, invoicing, and
> balance enforcement are all implemented as blocks that users can swap, extend,
> or replace. For example, invoice generation can use Meteridian's built-in
> invoicing block or be replaced by a Sink block that delegates to Zuora, Lago,
> or Stripe Billing. Payment collection is a pluggable block interface with
> built-in implementations for Stripe, Paddle, Adyen, and PayPal — and
> third-party providers can be added via the block marketplace (see
> [ADR-0010](../../docs/adr/0010-pluggable-payment-providers.md)).
