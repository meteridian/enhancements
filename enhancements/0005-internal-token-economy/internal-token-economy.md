...
- **Related:** METR-0002 (Platform Extensibility), METR-0013 (Unit Economics), ADR-0021 (Unlimited Custom Dimensions)
...
5. [Cost Allocation Rules](#5-cost-allocation-rules)
   - 5.5 [Custom Dimensions in Cost Allocation and Chargeback](#55-custom-dimensions-in-cost-allocation-and-chargeback)
...
This maps directly to Koku's existing distribution model:
...
### 5.5 Custom Dimensions in Cost Allocation and Chargeback

Metering events carry **unlimited custom dimensions** via CloudEvents extension
attributes with the `mtrn` prefix, stored in a JSONB column with GIN indexing
(see [ADR-0021](../../docs/adr/0021-unlimited-custom-dimensions.md)). This
capability transforms chargeback reporting and budget governance from a
tag-limited exercise into a fully flexible cost attribution system.

#### Beyond Fixed Tags

Traditional cost management platforms constrain chargeback to a small set of
pre-registered dimensions. Koku, for example, supports approximately 40 enabled
tag keys per provider type. AWS Cost Explorer limits cost allocation tags to 500.
Kubecost relies on Kubernetes labels, which are constrained by the label
vocabulary. These limits force organizations to choose which cost attribution
axes matter *before* they have data, and retroactive addition is expensive or
impossible.

Meteridian removes this constraint entirely. Any dimension attached to a
metering event — `customer_id`, `feature_name`, `deployment_region`,
`model_version`, `experiment_id`, `cost_center`, `contract_id`, or any
user-defined attribute — is immediately available for:

- **Cost allocation rules.** Allocation rules (Section 5.3) can match and
  group by any custom dimension, not just predefined tags. For example, a rule
  can allocate GPU costs by `model_version` and `customer_id` simultaneously.

- **Budget scoping.** Budget allocations (Section 3.2) can be scoped by
  arbitrary dimension combinations, not just the organizational hierarchy.
  A team can receive separate budget allocations for production and staging
  environments, or for different customer cohorts, using custom dimensions as
  the scoping key.

- **Chargeback drill-down.** Chargeback reports (Section 9.4) can break down
  costs by any combination of custom dimensions. A report for the ML team can
  show cost by model version, by training run, by dataset, and by customer —
  all in the same query.

- **Internal rate card differentiation.** Rate card entries (Section 3.4) can
  apply different internal rates based on custom dimensions. For example, GPU
  hours for production inference might cost 500 budget units, while GPU hours
  for experimentation might cost 200 budget units — differentiated by the
  `environment` custom dimension.

#### Allocation Rules with Custom Dimensions

The allocation rules engine (Section 5.3) supports custom dimension matching
natively:
...
```json
{
  "rules": [
    {
      "name": "ML costs by model version",
      "match": { "custom_dimension_exists": "model_version" },
      "allocate": { "method": "direct", "key": "custom_dimension:model_version" }
    },
    {
      "name": "Customer-specific GPU costs",
      "match": {
        "custom_dimension_equals": { "resource_class": "gpu" },
        "custom_dimension_exists": "customer_id"
      },
      "allocate": { "method": "direct", "key": "custom_dimension:customer_id" }
    },
    {
      "name": "Regional overhead distribution",
      "match": { "custom_dimension_exists": "deployment_region" },
      "allocate": {
        "method": "cpu_weighted",
        "scope": "custom_dimension:deployment_region"
      }
    }
  ]
}
...
#### Chargeback Report Drill-Down

Custom dimensions enable chargeback reports that go far beyond what fixed-tag
systems can provide:

```
GET /api/v1/chargeback/reports?account_id={uuid}&period=2026-06
    &group_by=custom_dimension:customer_id
    &group_by=custom_dimension:feature_name
    &filter[custom_dimension:environment]=production
```

This query produces a cost breakdown by customer and feature for production
workloads only — a report that would require manual spreadsheet work in
platforms limited to a fixed set of tag-based groupings.
...
**Drill down by custom dimensions (ADR-0021):**

Chargeback reports support grouping and filtering by any custom dimension
attached to metering events. This enables cost attribution granularity that
goes well beyond fixed-tag systems.
...
```json
{
  "account": { "id": "uuid", "name": "ML Team" },
  "period": { "start": "2026-06-01", "end": "2026-06-30" },
  "consumed": 187500,
  "dimension_breakdown": [
    {
      "dimensions": { "customer_id": "acme-corp", "feature_name": "image-processing" },
      "budget_units": 45000,
      "resource_breakdown": [
        { "resource_type": "gpu_a100_hour", "quantity": 80, "budget_units": 40000 },
        { "resource_type": "cpu_core_hour", "quantity": 500, "budget_units": 5000 }
      ]
    },
    {
      "dimensions": { "customer_id": "globex", "feature_name": "text-analysis" },
      "budget_units": 32000,
      "resource_breakdown": [
        { "resource_type": "gpu_a100_hour", "quantity": 55, "budget_units": 27500 },
        { "resource_type": "memory_gb_hour", "quantity": 900, "budget_units": 4500 }
      ]
    }
  ]
}
