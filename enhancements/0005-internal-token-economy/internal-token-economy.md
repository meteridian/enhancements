...
9. [API](#9-api)
10. [Graph-Based Cost Allocation (Moat Deepening)](#10-graph-based-cost-allocation-moat-deepening)
...
## 10. Graph-Based Cost Allocation (Moat Deepening)

### 10.1 Motivation

Current cost allocation uses flat tags/labels to attribute costs. Real infrastructure has hierarchical and many-to-many dependencies (shared databases, platform services, networking). A DAG (Directed Acyclic Graph) model captures these relationships and enables accurate multi-hop allocation.

### 10.2 Cost Flow Graph
...
const (
    ...
    NodePlatform       NodeType = "platform_service"
    NodeShared         NodeType = "shared_service"
...
Example graph:
```
[Physical Servers] --40%--> [K8s Platform]
[Physical Servers] --30%--> [Storage Service]
[Physical Servers] --30%--> [Network Service]
[K8s Platform]     --usage--> [Tenant A]
[K8s Platform]     --usage--> [Tenant B]
[Storage Service]  --GB--> [Tenant A]
[Storage Service]  --GB--> [Tenant B]
[Network Service]  --split--> [All Tenants]
```

### 10.3 Allocation Algorithms

- **Proportional flow:** Costs flow through edges proportional to weights
- **Step-down:** Sequential allocation from shared to specific (accounting standard)
- **Reciprocal:** Handles circular dependencies (iterative matrix solution)
- **Activity-based:** Weights derived from actual usage metrics

### 10.4 What-If Reallocation

Simulate changes to the graph without modifying production:
- "What if we move Team X off the shared database to dedicated?"
- "What if we add a new shared service layer?"
- Instant recomputation of all downstream allocations

### 10.5 API

```
GET  /api/v1/allocation/graph                   # Current cost graph
POST /api/v1/allocation/graph/simulate          # What-if simulation
GET  /api/v1/allocation/graph/nodes/{id}/flow   # Cost flow for a node
```
...
### 11.3 Shared Kubernetes Namespaces
...
### 11.4 Internal Rate Card Governance
...
### 11.5 Retrospective Adjustments
