...
13. [Graceful Recovery Paths (Moat Deepening)](#13-graceful-recovery-paths-moat-deepening)
...
## 13. Graceful Recovery Paths (Moat Deepening)

### 13.1 Motivation

Current enforcement focuses on the downward path (notify → throttle → block → terminate). The recovery path — what happens AFTER a customer pays or resolves the issue — is equally critical but underspecified. No competitor handles automated recovery well.

### 13.2 Recovery State Machine

```
terminated → (payment received) → pending_recovery → (grace check) → reinstating → active
blocked    → (payment received) → pending_recovery → (grace check) → reinstating → active
throttled  → (payment received) → unthrottling → active
```

### 13.3 Recovery Policy
...
type RecoveryPolicy struct {
    ...
    SLAGuarantee    *time.Duration  `json:"sla_guarantee,omitempty"`
}
...
type RecoveryStep struct {
    ...
    Rollback    string        `json:"rollback_on_failure,omitempty"`
}
```

### 13.4 SLA-Aware Grace Periods

Enterprise customers with SLAs get enhanced recovery:
- Payment failure grace period based on SLA tier (Gold: 72hr, Silver: 48hr, Bronze: 24hr)
- During grace: service continues, enforcement signals suppressed
- Automatic retry with exponential backoff before escalation
- Recovery SLA: "service restored within 15 minutes of payment confirmation"

### 13.5 Audit Trail

Every enforcement and recovery event is logged immutably:
...
Direction: "enforcement" (downward) or "recovery" (upward).

### 13.6 Progressive Re-enablement

For terminated services, recovery isn't instant — it's progressive:
1. Payment confirmed → mark `pending_recovery`
2. Verify no outstanding balance → `reinstating`
3. Re-provision resources (signal to METR-0015 Order Management)
4. Restore access policies (un-block in ext_authz/Limitador)
5. Mark `active`
6. Send recovery confirmation notification

Each step can fail independently with its own retry policy.
