...
- **Related:** METR-0004 (Credit and Token Billing), METR-0005 (Internal Budget Units), ADR-0004 (CloudEvents), ADR-0013 (Two-Layer Data Architecture), ADR-0019 (Multi-Cloud Cost Normalization), ADR-0020 (FOCUS v1.4 Interchange Format)
...
Meteridian: the ability to collect, normalize, and aggregate cost and usage data
from any cloud provider, SaaS service, or on-premise infrastructure into a
single unified view. Rather than limiting coverage to three hyperscalers,
Meteridian adopts a tiered provider model that combines native integrations with
the FOCUS v1.4 standard (ADR-0020) to support an unbounded number of providers.

**Tiered provider support model:**

- **Tier 1 (v1.0):** FOCUS generic ingestion + RHOAI (CloudEvents) + AWS +
  Azure + GCP — native source blocks with real-time metering
- **Tier 2 (v1.1):** IBM Cloud, Oracle Cloud, Alibaba Cloud, Huawei Cloud —
  native source blocks for broader cloud coverage
- **Tier 3 (v1.2+):** SaaS integrations — OpenAI, Anthropic, Datadog,
  Snowflake, Databricks, MongoDB, and others via API polling
- **Tier Custom (v1.0):** Any provider via FOCUS CSV upload or API ingestion —
  immediate onboarding without custom integration
...
- **FOCUS v1.4 ingestion** (ADR-0020) provides the generic onboarding path —
  any provider that exports FOCUS-format billing data can be ingested without
  a custom source block
...
### 10.1 Tiered Provider Support Model

Meteridian supports providers through a tiered model that balances breadth of
coverage with depth of integration:

| Tier | Release | Integration type | Providers |
|------|---------|-----------------|-----------|
| **Tier 1** | v1.0 | Native source blocks with real-time metering | RHOAI (CloudEvents), AWS, Azure, GCP, OCP |
| **Tier 2** | v1.1 | Native source blocks (API polling) | IBM Cloud, Oracle Cloud, Alibaba Cloud, Huawei Cloud |
| **Tier 3** | v1.2+ | API polling and webhook integrations | OpenAI, Anthropic, Datadog, Snowflake, Databricks, MongoDB, Confluent, Vercel, Cloudflare, GitHub |
| **Tier Custom** | v1.0 | FOCUS v1.4 CSV upload or API ingestion | Any provider that exports FOCUS-format billing data |

**Tier 1** providers have native Redpanda Connect source blocks that query
provider APIs in real-time and emit CloudEvents with full metadata. These are
the deepest integrations with support for real-time balance management.

**Tier 2** providers have native source blocks but with polling-based ingestion
(typically daily cost exports). They provide full cost visibility but not
real-time metering.

**Tier 3** providers are SaaS and AI platforms integrated via their usage and
billing APIs. Integration depth varies by provider API capabilities.

**Tier Custom** leverages the FOCUS v1.4 ingestion pipeline (ADR-0020) to
support any provider that can export billing data in FOCUS format. This
eliminates the need for custom integrations — operators upload a FOCUS-format
CSV or configure automated delivery to an S3-compatible bucket.

### 10.2 Phased Delivery

| Phase | Scope | Effort |
|-------|-------|--------|
| Phase 1 (PoC) | AWS Bedrock source block + on-premise RHOAI + FOCUS ingestion pipeline + unified reporting | 3-4 weeks |
...
| Phase 5 | Tier 2 providers (IBM Cloud, Oracle Cloud, Alibaba Cloud, Huawei Cloud) | 4-6 weeks |
| Phase 6 | Tier 3 SaaS integrations (OpenAI, Anthropic, Datadog, Snowflake, Databricks, MongoDB) | 6-8 weeks |
...
### 10.4 Redpanda Connect as the Integration Mechanism
...
- [ADR-0020](../../docs/adr/0020-focus-cost-data-interchange.md) — FOCUS v1.4
  as canonical cost data interchange format (enables any-provider ingestion)
