...
13. [Enterprise Discount Attribution (Moat Deepening)](#13-enterprise-discount-attribution-moat-deepening)
...
## 13. Enterprise Discount Attribution (Moat Deepening)

### 13.1 Motivation

Enterprise cloud customers have complex discount structures (AWS EDP, Azure MACC, GCP CUDs, Savings Plans) that are applied at the billing account level but need attribution to individual teams/projects for accurate chargeback.

### 13.2 Discount Types

| Provider | Discount Program | Mechanism |
|----------|-----------------|-----------|
| AWS | EDP (Enterprise Discount Program) | Flat % off all spend above commitment |
| AWS | Savings Plans | Commit to $/hr, get reduced rate on matching usage |
| AWS | Reserved Instances | 1yr/3yr commit for specific instance type |
| Azure | MACC (Microsoft Azure Consumption Commitment) | Commit to total spend, get discount tier |
| Azure | Reserved Instances | Same as AWS RI concept |
| GCP | CUDs (Committed Use Discounts) | Commit to vCPU/RAM quantities |
| GCP | SUDs (Sustained Use Discounts) | Automatic discount for sustained usage |

### 13.3 Attribution Methods

```go
type DiscountAttribution struct {
    Method          AttributionMethod `json:"method"`
    DiscountPool    uuid.UUID         `json:"discount_pool_id"`
    Beneficiaries   []Beneficiary     `json:"beneficiaries"`
}

type AttributionMethod string
const (
    Proportional   AttributionMethod = "proportional"
    FirstComeFirst AttributionMethod = "first_come_first_served"
    Priority       AttributionMethod = "priority_weighted"
    EvenSplit      AttributionMethod = "even_split"
)
```

- **Proportional:** Discount allocated by share of eligible spend (most common)
- **Priority:** Business-critical workloads get discount first, overflow at on-demand
- **First-come:** Time-ordered (whoever ran first in the billing period)
- **Even split:** Equal allocation regardless of usage (for shared platforms)

### 13.4 Savings Plan Matching

Algorithm to match Savings Plans to usage optimally:
1. Ingest Savings Plan inventory (type, commitment, utilization)
2. Match eligible usage to SP (by instance family, region, OS)
3. Attribute SP savings to the team whose workload was covered
4. Report: SP utilization %, coverage %, waste
...
type OptimizationRecommendation struct {
    ...
    CurrentSpend    decimal.Decimal `json:"current_monthly_spend"`
    RecommendedCommit decimal.Decimal `json:"recommended_commitment"`
    ProjectedSavings decimal.Decimal `json:"projected_monthly_savings"`
    BreakEvenMonths int             `json:"break_even_months"`
    Confidence      float64         `json:"confidence"`
}
```

Generate recommendations for: new RI/SP purchases, RI exchanges, SP renewals, CUD adjustments.
